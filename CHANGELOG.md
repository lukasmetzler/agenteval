...
- npm: GitHub redirects release downloads to `release-assets.githubusercontent.com` via HTTP. Added to allowed hosts and handle HTTP redirects from GitHub CDN domains.
...
- OpenCode harness adapter: `--harness opencode` with dedicated adapter. Reads `AGENTS.md`, auto-detected when `AGENTS.md` exists. Previously fell through to generic adapter, now has proper support with token parsing.
...
- npm: SHA256 integrity verification for downloaded binaries. Checksums are injected at publish time by the release workflow. Rejects corrupted or tampered downloads.
- npm: redirect validation, only follows HTTPS redirects to github.com, max 5 hops

## [0.8.3] - 2026-04-06

### Fixed

- npm: bin shim renamed to `.js` extension so socket.dev and other security scanners can analyze the source code
...
- README rewritten: npm as primary install, installation options table, CI integration section
- npm package page: full README with badges, platform table, config example
- Dependencies updated: glob 13, zod 4, biome 2, TypeScript 6
- Cleaned up 59 stale branches, CLAUDE.md updated

## [0.7.10] - 2026-04-05

### Fixed

- Install script: handle both minified and pretty-printed GitHub API JSON
- npm: first successful publish as `agenteval-cli` (npm rejected `agenteval` as too similar to `agent-eval`)

## [0.7.9] - 2026-04-05

### Fixed

- Release workflow: npm and Homebrew now run as independent jobs so one failing doesn't block the other
- npm publish uses `continue-on-error` until token is correctly configured
