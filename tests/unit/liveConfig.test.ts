...
describe("selectAndScoreRubrics", () => {
	test("all rubrics enabled with equal weight — average score", () => {
		const result = selectAndScoreRubrics(sampleFiles, cleanDiff, defaultConfig);
	...
	test("one rubric disabled — excluded from result and average", () => {
		...
		const result = selectAndScoreRubrics(sampleFiles, cleanDiff, config);
	...
	test("custom weights — weighted average", () => {
		...
		const result = selectAndScoreRubrics(manyFiles, cleanDiff, config);

		// weighted = (0.8*2 + 0.5*1 + 1.0*1) / (2+1+1) = 3.1/4 = 0.775 * 10 = 7.8
		expect(result.overallScore).toBe(7.8);
	});

	test("all rubrics disabled — overallScore 0 and empty rubrics", () => {
		const config: LiveReviewConfig = {
			rubrics: {
	...
	test("zero weight — contributes nothing to average", () => {
		const config: LiveReviewConfig = {
			rubrics: {
		...
		const result = selectAndScoreRubrics(files, cleanDiff, config);
	...
	test("no files — returns score 10 with no rubrics", () => {
		const result = selectAndScoreRubrics([], "", defaultConfig);
		expect(result.rubrics).toHaveLength(0);
	...
	test("all weights zero — overallScore 0", () => {
		const config: LiveReviewConfig = {
			rubrics: {
				scopeDiscipline: { enabled: true, weight: 0 },
